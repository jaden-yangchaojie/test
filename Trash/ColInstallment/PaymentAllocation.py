...
def ins_handler_had_accounting_allocation(statement_period,cur_statement_time, purchase_data_all, get_all_statement_data, amount):
    ...
    for get_pay_order in PaymentOrderList:
        ...
        if get_pay_order == "min_payment_late_interest":
            if amount > 0:
                        ...
                        for get_key in get_purchase_data_one.keys():
                            if get_key.count("purchase_"):
                                ...
                                if cur_statement_period in get_veryone_statement_list:
                                    ...
                                    if get_one_info["late_interest_cal"] == get_one_info["repaid_late_interest_cal"]:
                                        continue
                                    elif amount > 0:
                                        amount = amount - int(get_one_info["late_interest_cal"])
                                        if amount > 0:
                                            get_one_info = get_ins_repaid_info[get_pre_statement_period]
                                            get_one_info["repaid_late_interest_cal"] = get_one_info["late_interest_cal"]
                                        else:
                                            get_one_info["repaid_late_interest_cal"] = get_one_info[
                                                                                           "repaid_late_interest_cal"] - amount
                        ...
                        for get_key in get_purchase_data_one.keys():
                            if get_key.count("purchase_"):
                                ...
                                if cur_statement_period in get_ins_repaid_info.keys():
                                    get_one_info = get_ins_repaid_info[get_pre_statement_period]
                                    if get_one_info["repaid_prin"] == get_one_info["ins_total_prin"]:
                                        continue
                                    elif amount > 0:
                                        amount_less_0 = copy.deepcopy(amount)
                                        amount = amount - (int(get_one_info["ins_total_prin"])-int(get_one_info["repaid_prin"]))
                                        if amount >= 0:
                                            get_one_info = get_ins_repaid_info[get_pre_statement_period]
                                            get_one_info["repaid_prin"] = get_one_info["ins_total_prin"]
                ...
                for get_key in get_purchase_data_one.keys():
                    if get_key.count("purchase_"):
                        ...
                        for get_statement_period_one in get_veryone_statement_list:
                            ...
                            if int(get_one_info["ins_total_prin"]) == int(get_one_info["repaid_prin"]):
                                continue
                            elif amount > 0:
                                amount_less_0=copy.deepcopy(amount)
                                amount = amount - (int(get_one_info["ins_total_prin"])-int(get_one_info["repaid_prin"]))
