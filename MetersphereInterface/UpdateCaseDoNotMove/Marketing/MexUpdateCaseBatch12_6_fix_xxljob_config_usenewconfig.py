...
def only_find(get_data, insert_str_data):
    if get_data["type"] == "HTTPSamplerProxy" and get_data["enable"] == True:
        ...
        if str(get_data["path"]).count("/xxl-job-admin/jobinfo/trigger") > 0:
            # print(str(get_data['hashTree']))
            if str(get_data['name']).count("Job_CashBack_View_Notify") > 0 :
                get_body = get_data["body"]
                if str(get_body).count("useNewConfig")>0:
                    get_tmp = str(get_body).replace("\"useNewConfig\":\"new\",", "")
                    get_tmp = str(get_tmp).replace("\"new\"", "true")
