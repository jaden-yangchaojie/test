...
def only_find(get_data, insert_hash_tree_data):
    if get_data["type"] == "HTTPSamplerProxy" and get_data["enable"] == True:
        ...
        if str(get_data["path"]).count("/v1.0/credit/authorizations/posting") > 0 or str(get_data["path"]).count("/v1.0/credit/authorizations")>0:
            print(str(get_data["path"]))
            get_body = get_data["body"]
            if str(get_body).count("81628beb-7bab-4537-9dd1-462ae35c30aa")>0:
                get_tmp=str(get_body).replace("81628beb-7bab-4537-9dd1-462ae35c30aa","${accountId}")
