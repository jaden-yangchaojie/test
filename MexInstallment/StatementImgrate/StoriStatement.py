...
from StoriTrade import APPLY_NEW_Installment_RULES, PRINTradingListType, DayBalance, TradingStatus, TradingType, \
    StoriTrade, PaymentOrderList, PaymentOrderListFirstPrin, isInstallmentType, isNegativeType, positiveUnpaidFilter, \
    unpaidFilter, tradingListSortByTime
from StoriInstallment import Installment
# from logger import logger
from StoriBill import StoriBill
from StatementImgrate.NoUse.StoriLabel import StoriLabel, StoriDQFlag, StoriPaymentFlag, StoriChargingFlag, \
    StoriStatementFlag, StoriTradingFlag, StoriAdjustmentFlag, StoriMinimumFlag, StoriFreeIntCouponFlag, \
...
class Statement:
    ...
    def lackOfDQTrigger(self, date) -> bool:
        ...
        if lastStatement != None:
            if date > lastStatement.bill.DQTriggerDay and (
                    self.cif.DQEnabledDate == None or self.cif.DQEnabledDate < lastStatement.bill.DQTriggerDay):
    ...
    def handleRealTimePaymentBeforeCharging(self):
        ...
        # 如果上期账单有溢缴款，插入溢缴款
        if (self.lastStatement != None and self.lastStatement.overTrading and round_to_even(
                self.lastStatement.overTrading.leftAmount) > 0):
            ...
            for statement in statementList:
                for unpaidTrading in statement.unpaidTradingDic[tradingType]:
                    ...
                    for number in unpaidTrading.unpaidInMinimum:
                        ...
                        self.updateUnpaidData(trading, statementList[number - firstNumber], unpaidTrading, payment,
                                              tradingType)
    ...
    def handleHistoryPayment(self, trading: StoriTrade = None):
        ...
        for statement in statementList:
            ...
            for tradingType in PaymentOrderList:
                unpaidTradingList = statement.unpaidTradingDic[tradingType] + statement.unpaidTradingDicOfPreStatement[
                    tradingType]
                for unpaidTrading in unpaidTradingList:
    ...
    # payament : 还款金额
    # tradingType : 被还款交易类型
    def updateUnpaidData(self, negativeTrading: StoriTrade, minimumStatement, positiveTrading: StoriTrade, payment,
        ...
        while statement.bill.number != positiveTrading.billNumber:
            statement = statement.lastStatement
            if (minimumNumber and round_to_even(
                    statement.unpaidMinimum) != 0 and statement.bill.number >= minimumStatement.bill.number):
        ...
        if minimumNumber > 0:
            positiveTrading.unpaidInMinimum[minimumNumber] = round_to_even(
                positiveTrading.unpaidInMinimum[minimumNumber] - payment)
            assert (positiveTrading.unpaidInMinimum[minimumNumber] >= 0)
        assert (positiveTrading.leftAmount >= 0)

        print("账期：", minimumStatement.bill.statementDate if minimumStatement else statement.bill.statementDate,
              "金额：", payment)

        isOverDQ = self.cif.checkDQOverLimit()
        self.bill.recordPayment(negativeTrading, positiveTrading, payment, minimumNumber=minimumNumber,
                                isOverDQ=isOverDQ)
    ...
    def handleInstallmentTrading(self, tid, data: dict):
        ...
        # 调整交易
        adjustmentTrading = StoriTrade(TradingType.ADJ_NEGATIVE, installment.principal / 100,
                                       effective_time=self.curTime)
    ...
    def handleOriginTrading(self, amount, tradingType=TradingType.PRIN, isPomelo=True, tid=None, sync=True,
                            curTime=None, installments=None):
        # 检查交易是否可以进行
    ...
    def handlePostingTrading(self, tid, trading: StoriTrade = None, amount=0, isPomelo=True, curTime=None,
                             installments: dict = None):
        if trading == None:
    ...
    def calOverPayment(self):
        ...
        overTrading = StoriTrade(tradingType=TradingType.PAYMENT, status=TradingStatus.POSTED,
                                 effective_time=self.curTime, amount=overAmount)
    ...
    def genADBList(self, endDate=None):
        ...
        # 计算每天的ADB
        self.bill.adbList = DayBalance.genBalanceList(self.bill.startDate, endDate, originBalance, tradingDic,
                                                      isADB=True)
            ...
            for tradingType in PRINTradingListType:
                ...
                for prinTrading in prinTradingList:
                    ...
                    for number in prinTrading.unpaidInMinimum:
                        canAllocatedNotInMinimum = round_to_even(
                            canAllocatedNotInMinimum - prinTrading.unpaidInMinimum[number])
                    ...
                    if canAllocatedNotInMinimum > 0:
                        ...
                        self.bill.minimumDic[tradingType] = round_to_even(
                            self.bill.minimumDic[tradingType] + allocatedAmount)
