...
# usageCount 必填
benefit_batch_no = "26032400100100333300000000000000"
benefit_name = '自动化测试勿动-redemption-'
...
crowd_no = "26032412345678910111200000000000"
crowd_rules = json.dumps(
    {"productCode": "STC002", "openDate": "2024-10-01T12:00:00", "openEndDate": "2024-10-01T12:59:59"})
activity_no = "MK20260228000000"
#不改动##########
task_name = activity_no + "_BENEFIT_TASK"
start_time_activity = start_time
end_time_activity =end_time
#百分比#########
# preferential_rules = json.dumps({"singleAmountLimit": 10000, "calculateMethod": "percent", "calculateParam": "0.07", "rewardChannel": "REDEMPTION"})
# preferential_rules = json.dumps({"singleAmountLimit": 10000,  "calculateMethod": "percent", "calculateParam": "0.07", "rewardChannel": "REDEMPTION"})
...
#返回固定金额
preferential_rules = json.dumps({ "calculateMethod": "fixAmount", "calculateParam": "100", "rewardChannel": "REDEMPTION"})
...
mk_bf_instance = (
            ...
            "('STMXFINCORE0','MXCC20220000','000000','" + benefit_batch_no + "','" + benefit_name + "','CASHBACK','ACTIVE','STC002','Credit','tester','tester','自动化测试勿动-提现权益化','2025-08-01 03:43:47','2025-08-01 09:43:47','2025-08-01 03:43:47','2025-08-01 09:43:47',null);")
...
mk_activity = (
            ...
            "('STMXFINCORE0','MXCC20220000','" + activity_no + "','REDEMPTION','BENEFIT autotest提现权益化-自动化测试勿动','benefit 提现权益化-自动化测试','ACTIVE',null,'"+activity_rules+"','','" + crowd_no + "','system','system','" + start_time_activity + "','" + end_time_activity + "','2025-08-19 22:01:50','2025-08-20 04:01:50','2025-08-19 22:01:50','2025-08-20 04:01:50',null,'STC002','CREDIT',null,null,null,'" + activity_no + "_audience',null,null);")
...
mk_activity_task = (
            ...
            "('STMXFINCORE0','MXCC20220000','" + task_name + "','" + benefit_name + "','benefit Autotest','DEFAULT','','{}','" + activity_no + "','REDEMPTION','" + benefit_batch_no + "',null,'" + start_time + "','" + end_time + "','','"+'{"autoConfig":{"autoRunning": true,"autoComplete": true,"autoReward":true}}'+"','2025-08-18 20:40:57','2025-08-19 02:40:57','2025-08-18 20:40:57','2025-08-19 02:40:57');")
...
#                     "('STMXFINCORE0','MXCC20220000','1111','BENEFIT','"+get_one+"','4','自动化测试勿动-提现权益化','2025-09-10 21:45:07','2025-09-10 21:45:07','2025-09-10 21:45:07','2025-09-10 21:45:07');")
#
#     print(mk_exclusion_group + "\n")

del_mk_bf_instance='delete from configA1.`mk_bf_instance` where benefit_batch_no="'+benefit_batch_no+'" limit 1;'
del_mk_bf_instance_rules='delete from configA1.`mk_bf_instance_rules` where benefit_batch_no="'+benefit_batch_no+'" limit 1;'
del_mk_bf_instance_stock='delete from configA1.`mk_bf_instance_stock` where benefit_batch_no="'+benefit_batch_no+'" limit 1;'
del_mk_activity='delete from configA1.`mk_activity` where activity_no="'+activity_no+'" limit 1;'
del_mk_crowd='delete from configA1.`mk_crowd` where crowd_no="'+crowd_no+'" limit 1;'
del_mk_common_param='delete from configA1.`mk_common_param` where activity_no="'+activity_no+'" limit 1;'
del_mk_activity_task='delete from configA1.`mk_activity_task` where activity_no="'+activity_no+'" limit 1;'

print(del_mk_bf_instance)
print(del_mk_bf_instance_rules)
print(del_mk_bf_instance_stock)
print(del_mk_activity)
print(del_mk_crowd)
print(del_mk_common_param)
print(del_mk_activity_task)
